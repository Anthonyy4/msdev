...
exports.run = (client, message, args) => {
  ...
  const embed = new Discord.RichEmbed()
    ...
    if (!mslogon) return message.channel.send(`**[ :lock: | Action Mute ]** <@${message.author.id}>, \`${user.id} ${user.username}\` | ${user} has been **Muted** for **[ \`${reason}\` ]**`).then(message.guild.member(user).removeRole(memberRole)).then(message.guild.member(user).removeRole(certiRole)).then(message.guild.member(user).addRole(muteRole));      

    if (mslogon) return message.channel.send(`**[ :lock: | Action Mute ]** <@${message.author.id}>, \`${user.id} ${user.username}\` | ${user} has been **Muted** for **[ \`${reason}\` ]**`).then(client.channels.get(modlog.id).send({embed})).then(message.guild.member(user).removeRole(memberRole)).then(message.guild.member(user).removeRole(certiRole)).then(message.guild.member(user).addRole(muteRole));
